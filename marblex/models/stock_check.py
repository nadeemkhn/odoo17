...
class CheckSaleOrder(models.Model):
    ...
    no_order_line_value = fields.Boolean(
        string="Stock Check",
        compute="_compute_no_order_line_value",
        store=True,
    )

    @api.depends('check_order_id.operation_type')
    def _compute_no_order_line_value(self):
        for order in self:
            order.no_order_line_value =  any(line.operation_type for line in order.check_order_id)
