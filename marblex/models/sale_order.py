...
class SaleOrderShapeExt(models.Model):
    ...
    beveling_charges = fields.Char(string="Charges", default="Total Beveling Charges")
    beveling_pcs = fields.Float(string="Total Pcs", compute="_compute_beveling_totals")
    beveling_qty = fields.Float(string="Total Qty", compute="_compute_beveling_totals")
    ...
    def action_generate_report(self):
        ...
        return self.env.ref('marblex.albari_sale_action_id').report_action(self)
    #
    # @api.onchange('order_line.unit')
    # def _onchange_methods_unit(self):
    #     for record in self:
    #         record.beveling_pcs = sum(record.order_line.mapped('unit'))

    # @api.depends('order_line.unit')
    # def _compute_beveling_total(self):
    #     for record in self:
    #         record.beveling_pcs = sum(record.order_line.mapped('unit'))
            ...
            stock_delivery_vals = {
                ...
                'check_order_id': [
                    (0, 0, {
                        ...
                        'shape_id': line.shape_id.id if line.shape_id else False,
                        'shape_id_image': line.shape_id.image if line.shape_id and line.shape_id.image else False,# Related field works automatically
                        'remarks': line.remarks,
    ...
    def _compute_beveling_totals(self):
        for order in self:
            ...
            print(f"Computing beveling totals for order: {order.name}")  # Debugging statement

            for line in order.order_line:
                print(f"Processing line: {line.id}, Unit: {line.unit}")  # Debugging statement
            ...
            print(
                f"Total Pcs: {total_pcs}, Total Qty: {total_qty}, Total Amount: {total_amount}")  # Debugging statement
